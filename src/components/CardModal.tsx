...
import { X, Plus, Trash2, Calendar, ListTodo, Palette, Tag, Briefcase, Paperclip, Link2, FileText, Upload, Download, ExternalLink, Share2, CheckCircle, Repeat, Flame } from 'lucide-react';
import { Card, Priority, Subtask, Attachment } from '../types';
import { toDateStr, getStreak } from '../utils/recurring';
...
export default function CardModal({
  ...
  // Recurring (daily) demand state
  const [isRecurring, setIsRecurring] = React.useState(false);
  const [lastCompletedDate, setLastCompletedDate] = React.useState<string | undefined>(undefined);
  ...
  React.useEffect(() => {
    if (card) {
      ...
      setLastCompletedDate(card.lastCompletedDate);
      setCompletedDates(card.completedDates || []);
      ...
      setLastCompletedDate(undefined);
      setCompletedDates([]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      ...
      {/* Modal Box */}
      <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden flex flex-col z-10 transition-all max-h-[95vh] sm:max-h-[90vh]">

        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4.5 bg-slate-50 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2 min-w-0">
            <span className="p-1.5 bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 rounded-lg shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                {boards && (
                  <select
                    ...
                    className="w-full sm:w-1/2 px-3 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm text-slate-700 dark:text-slate-300 transition-all font-medium"
                  >
                    {boards.map((b) => (
                ...
                <select
                  ...
                  className="w-full sm:flex-1 px-3 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm text-slate-700 dark:text-slate-300 transition-all font-medium"
                >
                  {columns
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-3">
            <label className="flex items-center justify-between cursor-pointer gap-3">
              <span className="text-xs font-bold text-slate-600 dark:text-slate-300 flex items-center gap-1.5 uppercase tracking-wider">
                <Repeat className="w-3.5 h-3.5" /> Demanda Recorrente (Diária)
              </span>
              <button
                type="button"
                role="switch"
                ...
                className={`relative w-9 h-5 rounded-full transition-colors cursor-pointer shrink-0 ${
                  isRecurring ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-700'
                }`}
              >
                <span
            ...
            <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-relaxed">
              Ative para tarefas que devem ser refeitas todos os dias (ex: postar vídeos, redes sociais). O cartão fica pendente de novo automaticamente a cada novo dia — você só marca como feito a cada execução.
            </p>

            {isRecurring && (
              <div className="flex items-center flex-wrap justify-between gap-2 pt-2.5 border-t border-slate-200/70 dark:border-slate-800">
                <span className="flex items-center gap-1 text-[11px] font-bold text-amber-600 dark:text-amber-400">
                  <Flame className="w-3.5 h-3.5" /> {getStreak(completedDates)} dia(s) em sequência
                </span>
                <button
                  type="button"
                  onClick={handleToggleDoneToday}
                ...
                >
                  <CheckCircle className="w-3.5 h-3.5" /> {doneToday ? 'Feito Hoje' : 'Marcar como Feito Hoje'}
                </button>
              </div>
            )}
          </div>
        ...
        {/* Footer Actions */}
        <div className="px-4 sm:px-6 py-3.5 sm:py-4 bg-slate-50 dark:bg-slate-800/20 border-t border-slate-100 dark:border-slate-800 flex items-center flex-wrap justify-between gap-2">
            ...
            {card && (
              <>
                <button
                  ...
                  className="px-2.5 sm:px-3.5 py-2 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5"
                ...
                <button
                  ...
                  className="px-2.5 sm:px-3.5 py-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5"
                  title="Excluir Cartão"
                >
                  <Trash2 className="w-4 h-4" /> <span className="hidden xs:inline">Excluir</span>
                </button>
              </>
            )}
          </div>
          ...
          <div className="flex gap-1.5 sm:gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-3 sm:px-4 py-2 bg-white dark:bg-slate-950 hover:bg-slate-100 dark:hover:bg-slate-850 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold transition-all cursor-pointer"
            >
              Cancelar
            </button>
            <button
              type="button"
              onClick={handleSave}
              className="px-3.5 sm:px-4.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-600/15 cursor-pointer"
            >
              Salvar Cartão
            </button>
          </div>
